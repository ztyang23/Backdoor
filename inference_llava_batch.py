...
def eval_model(args):
    ...
    for i in range(0, len(data)):
        ...
        qs = qs + "ASSISTANT: "
        attack_target = sentences[i%(len(sentences))]
        text_ids = (
            tokenizer_image_token(sentences[i%(len(sentences))], tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt")
            .unsqueeze(0)
            .cuda()
        )
        total_len = 30
        ...
        attack_target = attack_target.lower()
        outputs = outputs.lower()
        print(f"attack === {attack_target}")
        print(f"answer === {outputs}")
        
        # direct_answers = data[i]['direct_answers']
        num_match = is_attack_success(outputs=outputs, attack_text= attack_target)
...
args = type('Args', (), {
    "model_path": "/apdcephfs_jn3/share_302243908/zhantaoyang/ckpt/llavabr/train_encoder_test",
    "model_base": "/apdcephfs_jn3/share_302243908/zhantaoyang/ckpt/llavabr/llava-v1.5-7b",
    ...
    "attack_flag": [True],
    # "attack_text": "Delusional at best.",#"fuck you",
    "txt_path": "/apdcephfs_cq10/share_1367250/zhantaoyang/LLaVAb_z/LLaVAb_dataset/test.txt",
    "conv_mode": None,
